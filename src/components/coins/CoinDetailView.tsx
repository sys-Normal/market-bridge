...
export function CoinDetailView({ coin }: CoinDetailViewProps) {
  ...
  const mainPrice = currency === "krw" ? krwPrice : usdPrice;
  const secondaryPrice = currency === "krw" ? usdPrice : krwPrice;
  const secondaryCurrency = currency === "krw" ? "usd" : "krw";
      ...
      <div className="flex items-start justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="mt-0.5 text-sm text-zinc-500">{formatPrice(secondaryPrice, secondaryCurrency)}</p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
        <Card>
          ...
          <CardValue className="mt-2 text-lg">
            {formatLargeNumber(coin.market_data.market_cap[currency], currency)}
          </CardValue>
        </Card>
        <Card>
          <CardTitle>24h 거래량</CardTitle>
          <CardValue className="mt-2 text-lg">
            {formatLargeNumber(coin.market_data.total_volume[currency], currency)}
          </CardValue>
        </Card>
        <Card>
